  ...
  return (
    ...
    >
        ...
        >
          <div className="space-y-4">
            ...
            <h2 className="md:text-3xl text-xl font-bold tracking-tight text-muted-foreground/80">
              ...
              into <span className="text-foreground">Human-Centric</span>{' '}
              experiences.
            </h2>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:pt-12">
        ...
        <motion.div
          variants={fadeInUp}
          className="lg:col-span-12 md:space-y-8 space-y-4 md:pt-12"
        >
            ...
            {skills.map((category, idx) => (
              <motion.div key={idx} whileHover={{ y: -10 }} className="group">
                <Card className="h-full border-border bg-card/40 backdrop-blur-md rounded-2xl overflow-hidden transition-all duration-500 group-hover:border-blue-500/30 group-hover:shadow-[0_0_40px_-15px_rgba(59,130,246,0.3)]">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex -space-x-2">
                        {category.icons.map((Icon, i) => (
                          <div
                            key={i}
                            className="p-2 rounded-full bg-background border border-border text-blue-500"
                          >
                            <Icon size={16} />
                          </div>
                        ))}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {category.items.map((item, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 text-[10px] font-black uppercase tracking-wider bg-muted/50 border border-border/50 rounded-lg text-muted-foreground group-hover:text-foreground transition-colors"
                          >
                            {item}
                          </span>
                        ))}
                  ...
                  <div
                    className={`absolute bottom-0 left-0 h-1 w-full bg-linear-to-r ${category.color} opacity-0 group-hover:opacity-100 transition-opacity`}
                  />
                </Card>
              </motion.div>
            ))}
      ...
      {/* <motion.div
            ...
            >
                <a href="#projects" onClick={(e) => { e.preventDefault(); document.getElementById('projects')?.scrollIntoView({ behavior: 'smooth' }); }} className="group inline-flex items-center gap-4 md:text-4xl text-2xl font-black tracking-tighter hover:text-blue-500 transition-colors">
                    Explore my work
